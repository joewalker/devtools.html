...
function addBreakpoint(location, condition) {
  return (dispatch, getState) => {
    ...
    let bp = _getOrCreateBreakpoint(getState(), location, condition);
    ...
    return dispatch({
      ...
      [PROMISE]: Task.spawn(function*() {
        ...
        let [response, bpClient] = yield rdpInvoke(sourceClient, sourceClient.setBreakpoint, {
...
function removeAllBreakpoints() {
  return (dispatch, getState) => {
    let breakpoints = getBreakpoints(getState());
    let activeBreakpoints = breakpoints.filter(bp => !bp.disabled);
...
function setBreakpointCondition(location, condition) {
  return (dispatch, getState) => {
    ...
    return dispatch({
      ...
      condition: condition,
      [PROMISE]: Task.spawn(function*() {
        let newClient = yield bpClient.setCondition(gThreadClient, condition);
