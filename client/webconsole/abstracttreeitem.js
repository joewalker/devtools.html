...
AbstractTreeItem.prototype = {
  ...
  expand: async function() {
    ...
    await this._toggleChildren(true);
  ...
  collapse: async function() {
    ...
    await this._toggleChildren(false);
   ...
   */
  _toggleChildren: async function(visible) {
    if (visible) {
      if (!this._populated) {
        await this._populateSelf(this._childTreeItems);
  ...
  _constructTargetNode: function() {
    ...
    let arrowNode = this._arrowNode = document.createElement("div");
